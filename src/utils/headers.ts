...
export function extractHeadersFromPath(event: H3Event): Record<string, string> {
  const headers: Record<string, string> = {};
  const originalUrl = event._path;
  const searchParams = new URLSearchParams(originalUrl);

  for (const [key, value] of searchParams) {
    // Exclude the 'destination' parameter, assuming it's a header
    if (key !== '/?destination') {
      headers[key] = value;
    }
  }
